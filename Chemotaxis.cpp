...
inline double bit_to_prob(const std::vector<bool> &bit_vec){
  int accumulator = 0;
...
void Chemotaxis::runWorldSolo(std::shared_ptr<Organism> org, bool analyse, bool visualize, bool debug){
  ...
  for(int t = 0; t != eval_ticks; ++t){
    ...
    //Will involve a vector of 'bits' (really ints), convert to ulong, double divide (resulting long +1) by 256.
